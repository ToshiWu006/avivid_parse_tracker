from update_banner_revenue import prepare_segment_revenues
...
if __name__ == "__main__":
    ...
    parser.add_argument("-is", "--is_save", help="update to table or not, format: 'T'(and none) or 'F'(and others)")
    ...
    for date_time in [datetime_start + datetime.timedelta(hours=dh) for dh in range(n_hours)]:
        ...
        print(date, hour)
        ## 11 events
        ## ['load', 'leave', 'timeout',python  'addCart', 'removeCart', 'purchase', 'sendCoupon', 'acceptCoupon', 'discardCoupon', 'enterCoupon', 'acceptAf']
        results = prepare_segment_revenues(date, hour, is_save=is_save)
