        ...
        if type_cal_cost==1: ## use coupon column to directly calculate cost (coupon column is value)
            query = f"""
                    ...
                    WHERE
                        ...
                        AND web_id = '{web_id}' AND coupon!=0 AND product_price>0
            ...
            query = f"""
                    ...
                    FROM
                        ...
                        WHERE
                            date_time BETWEEN '{activity_start}' AND '{activity_end}'
                            AND web_id = '{web_id}' AND product_price>0
                        GROUP BY uuid , session_id, timestamp
                        HAVING SUM(product_price * product_quantity) > {coupon_price_limit}) AS b ON a.uuid = b.uuid
            ...
            query = f"""
                    ...
                    FROM
                        ...
                        WHERE
                            date_time BETWEEN '{activity_start}' AND '{activity_end}'
                            AND web_id = '{web_id}' AND avivid_coupon='1' AND product_price>0
                        GROUP BY uuid , session_id, timestamp
                        HAVING SUM(product_price * product_quantity) > {coupon_price_limit}) AS b ON a.uuid = b.uuid
                            AND a.session_id = b.session_id
                        GROUP BY uuid , session_b) AS temp
...
def fetch_used_coupons(web_id, coupon_id, coupon_type, coupon_cost, coupon_amount, coupon_price_limit,
    ...
    for date in date_list:
        date_start = date
        date_end = datetime_to_str(to_datetime(date) + datetime.timedelta(days=1))
        df_used = fetch_coupon_used_revenue_cost(web_id, coupon_id, coupon_type, coupon_cost, coupon_amount, coupon_price_limit,
                                                 date_start, date_end, type_total_price, type_cal_cost)
...
def fetch_stat_by_id(web_id, coupon_id, coupon_type, coupon_cost, coupon_amount, coupon_price_limit,
    ...
    df_used = fetch_used_coupons(web_id, coupon_id, coupon_type, coupon_cost, coupon_amount, coupon_price_limit,
                                 date_start, date_end, type_total_price, type_cal_cost)
    df = pd.concat([df_accept.set_index('date_time'), df_imp.set_index('date_time'), df_used.set_index('date_time')], axis=1)
