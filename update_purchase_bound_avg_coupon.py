...
if __name__ == "__main__":
    ...
    # for web_id in web_id_list:
    #     ## update addfan_stat and cdp_tracking_settings table
    #     # df_stat = main_update_avg_shipping_purchase_max_coupon(web_id, max_n_coupon, update_db=True)
    #
    #
    #     ## update avg_n_coupon of each coupon in running (shared the same model)
    #     df_n_coupon = update_avg_coupon_a_day(web_id, update_db=False)
    #     avg_n_coupon = int(df_n_coupon['avg_n_coupon'])
    #     ## update lower_bound, model_precision, estimate_n_coupon
    #     df_activity_param = main_update_purchcase_bound(web_id, avg_n_coupon, data_use_db=True, update_db=True, plot_n_cum=False)
    #
    #
